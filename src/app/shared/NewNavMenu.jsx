...
const NewNavMenu = (props) => {
  ...
  function checkPathname() {
    if (pathname === "/") {
      ...
      console.log("pathname is not /");
    }
  }

  return (
    <NavigationMenu.Root className={props.className} delayDuration={100}>
      <NavigationMenu.List className="mx-12">
        <NavigationMenu.Item className={` ${pathname === "/" ? "md:visible invisible md:" : "invisible"}`}>
          <NavigationMenu.Trigger className=" max-w-fit ">
            <Link href="/" className={`link ${pathname === "/" ? "active" : ""}`}>
              <svg width="45" height="45" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={checkPathname} className="md:w-full scale-90">
                <path
          ...
          <NavigationMenu.Content className="flex flex-col justify-center items-center gap-1">
            <Link href="/#top" className={`link ${pathname === "/" ? "active" : ""}`} onClick={checkPathname}>
